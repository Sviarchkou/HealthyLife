...
{
    public class DailyElementController : DefaultController
    {
        ...
        public async Task<List<DailyElement>> select(User user)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                dailyElement.date = (DateTime)row[3];
        ...
        public async Task<DailyElement> insertDailyElement(User user, Element element)
        {
            ...
            dailyElement.date = DateTime.Now;
            string date = $"{DateTime.Now.Year}-{DateTime.Now.Month}-{DateTime.Now.Day}";
        ...
        public async Task updateDailyElement(DailyElement dailyElement)
        {
            ElementController elementController = new ElementController();
            await elementController.updateElement(dailyElement.element);
        }
    }
}
